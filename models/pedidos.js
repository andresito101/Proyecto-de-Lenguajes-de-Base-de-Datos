...
async function createPedidoSuministro(pedidoSuministro) {
    ...
    try {
        ...
        const result = await connection.execute(
            'INSERT INTO PEDIDOS_SUMINISTROS (ID_Pedido, ID_Suministro, Cantidad, Fecha_Pedido) VALUES (:ID_Pedido, :ID_Suministro, :Cantidad, :Fecha_Pedido)',
            [pedidoSuministro.ID_Pedido, pedidoSuministro.ID_Suministro, pedidoSuministro.Cantidad, pedidoSuministro.Fecha_Pedido],
            { autoCommit: true }
        );
        return result;
    } catch (err) {
...
async function updatePedidoSuministro(pedidoSuministro) {
    ...
    try {
        connection = await getConnection();
        const result = await connection.execute(
            'UPDATE PEDIDOS_SUMINISTROS SET Cantidad = :Cantidad, Fecha_Pedido = :Fecha_Pedido WHERE ID_Pedido = :ID_Pedido AND ID_Suministro = :ID_Suministro',
            [pedidoSuministro.Cantidad, pedidoSuministro.Fecha_Pedido, pedidoSuministro.ID_Pedido, pedidoSuministro.ID_Suministro],
            { autoCommit: true }
        );
        return result;
    } catch (err) {
